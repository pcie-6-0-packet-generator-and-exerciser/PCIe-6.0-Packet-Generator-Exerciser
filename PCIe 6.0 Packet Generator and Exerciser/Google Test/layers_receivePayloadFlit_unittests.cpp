...
#define DEFAULT_MEM_WRITE32_TLP_PARAMS 1, dynamicBitsetPayload, 0, 0, 0, 0, 0
...
TEST(ReceivePayloadFlitTestSuite, SingleTLPWithoutDataPayload) {
	// Set up default credit parameters
	DEFAULT_CREDIT_PARAMS;
	Globals globals(DEFAULT_CREDIT_VALUES);
...
TEST(ReceivePayloadFlitTestSuite, SingleTlpWithDataPayload) {
	DEFAULT_CREDIT_PARAMS;
	Globals globals(DEFAULT_CREDIT_VALUES);
	...
	auto flits = sendOnQueueFlits.popAll();
	//flits.push(flit);
	...
	EXPECT_EQ(receivedTlp->creditConsumedType, Dllp::CreditType::P);
	EXPECT_EQ(receivedTlp->headerConsumption, 1);
